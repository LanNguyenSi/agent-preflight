...
Design and implement the capability for: local CI simulation via act subprocess.

## Problem

The product cannot satisfy its initial scope until local CI simulation via act subprocess exists as a reviewable, testable capability.

## Solution

Add a focused module for local CI simulation via act subprocess that matches the recommended modular monolith and keeps integration boundaries explicit.
...
- [ ] The local CI simulation via act subprocess capability is available through the intended application surface.
- [ ] Core validation, error handling, and persistence for local CI simulation via act subprocess are covered by tests.
