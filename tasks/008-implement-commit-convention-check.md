# Task 008: Implement commit convention check
...
Design and implement the capability for: commit convention check.

## Problem

The product cannot satisfy its initial scope until commit convention check exists as a reviewable, testable capability.

## Solution

Add a focused module for commit convention check that matches the recommended modular monolith and keeps integration boundaries explicit.
...
- [ ] The commit convention check capability is available through the intended application surface.
- [ ] Core validation, error handling, and persistence for commit convention check are covered by tests.
