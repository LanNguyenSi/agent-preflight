...
| Test | `test` | Broken test suites | `npm test`, `pytest`, `phpunit`, `mvn test`, `gradle test` | `fail` when tests fail; `skip` for the auto-detected `npm test` when every failing package is unbuilt on disk (it has a `build` script of its own, a declared artifact is missing) and its own output names a path that resolves either to that missing artifact itself or to something in that package's build-output directory that is likewise NOT on disk. A path that is present is never evidence of a missing build, which is what keeps a partially built package (one declared artifact its build never emits, the rest built) a blocking `fail`. See the README's "Build-required test classification" section |
