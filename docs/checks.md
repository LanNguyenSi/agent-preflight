...
| Test | `test` | Broken test suites | `npm test`, `pytest`, `phpunit`, `mvn test`, `gradle test` | `fail` when tests fail; `skip` for the auto-detected `npm test` when the failure's own evidence shows a missing build artifact instead of a real bug (see the README's "Build-required test classification" section) |
