...
| Test | `test` | Broken test suites | `npm test`, `pytest`, `phpunit`, `mvn test`, `gradle test` | `fail` when tests fail; `skip` for the auto-detected `npm test` when every failing package is unbuilt on disk (it has a `build` script of its own, a declared artifact is missing), its build-output directory is absent or empty, and its own output names a path that resolves either to that missing artifact itself or to something in that directory that is likewise NOT on disk. An output directory holding ANY entry means a build ran and did not produce the artifact, so a partially built package (one declared artifact its build never emits, the rest built) stays a blocking `fail` naming both the directory and the artifact -- a stale partial output directory included. See the README's "Build-required test classification" section |
