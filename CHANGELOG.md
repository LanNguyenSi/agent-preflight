...
- **`npm audit` in the audit check is now bounded and reports an unreachable registry as `skip`, not `warn`**. The npm branch of `runAuditChecks` (`src/checks/audit.ts`) previously ran `npm audit --json` with no timeout and read only its exit code and `metadata.vulnerabilities`, so a registry advisory-endpoint outage (a stalled request, or an `npm error audit endpoint returned an error` response) could hang the whole preflight run and, once it did return, was misread as `warn` rather than "not evaluated". The npm call now goes through an exported `npmAuditRunner` object with a 90s timeout; a timeout, a network-level errno (`ENOTFOUND`/`ECONNREFUSED`/`ECONNRESET`/`ETIMEDOUT`/`EAI_AGAIN`/`ECONNABORTED`/`EPROTO`/`ERR_SOCKET_TIMEOUT`), npm's own endpoint-error message, or a JSON error envelope whose `error.code` is one of those or an HTTP 5xx/429 (`E502`/`E503`/`E504`/`E429`) now classify as a new `skip` outcome, with a message naming the cause and one `limitations` entry — `ready` stays unaffected and the check is not counted toward confidence the way a `pass` is. Real audit findings keep the existing pass/fail/warn behavior unchanged, and a *local* npm failure with no registry involvement (e.g. `ENOLOCK` — no lockfile present, the normal outcome for yarn/pnpm repos or one not yet installed) stays a `warn` naming that failure instead of being mislabeled unreachable. `npmAuditRunner` is a documented test seam: the test suite spies on it instead of shelling out to the real registry, and CI's `npm ci` now runs with `--no-audit --no-fund` so its own implicit audit no longer depends on registry reachability either.

  Fix-round (round-1 review): the classifier previously matched the outage markers against stdout+stderr combined even when npm's own report had already parsed, so a real `fail`/`pass` result could be mislabeled `skip` if an unrelated line mentioned one of those tokens (in stderr, or inside the report body itself, e.g. an advisory titled after a CVE in one of the network errnos) — and any non-zero exit with an unparsed JSON error envelope was treated as unreachable regardless of `error.code`, so a local failure like `ENOLOCK` was misreported as a registry outage. The classifier now judges a parsed vulnerability report on its own counts only, unconditionally, before ever consulting markers or an envelope's code; an envelope is only "unavailable" when its `error.code` is one of the registry-attributable codes above, everything else (`ENOLOCK`, `EUSAGE`, an unrecognized code, ...) is `warn`. Six new tests in `tests/audit.test.ts` pin this: four negative controls reproducing the mislabeled cases above (a real fail with a marker-bearing stderr line, a real pass with a marker-bearing stderr line, the `ENOLOCK` envelope, and a marker token that only appears inside the parsed report body), plus two covering the envelope-code split (an HTTP 5xx envelope with no `summary`) and a non-zero exit with neither JSON nor marker text.
