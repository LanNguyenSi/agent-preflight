...
- **`PREFLIGHT_LOG_DIR` environment variable overrides the default
  ...
  `PREFLIGHT_LOG_DIR` when set to an absolute path, sitting between
  `.preflight.json`'s `logDir` (still highest precedence) and the
  `~/.agent-preflight/logs` default. A relative value is ignored with a
  warning naming the variable; an empty value is treated as unset. Lets a
  CLI run against a scratch fixture, or a parallel worktree sharing `$HOME`
  with other checkouts, isolate its failure logs without a `.preflight.json`
  edit. See README's log directory section for the full precedence table.
...
- **`preflight run --json` and `preflight batch --json` no longer truncate a large result envelope when stdout is a pipe** (task 0089e6f5). Both commands' `--json` path called `console.log(JSON.stringify(result))` immediately followed by `process.exit()`; writing to a pipe is asynchronous, so a payload bigger than the OS pipe buffer (commonly 64 KB / 65536 bytes on both Linux and macOS) only had its first chunk copied into the kernel synchronously before `process.exit()` tore the process down, and a reader on the other end (`preflight run --json | some-consumer`) saw a cut-off, unparseable JSON document -- confirmed directly: the same payload written to a regular file (no pipe-buffer limit there) came through complete, while the piped copy stopped at exactly 65536 bytes. The T-008 pathological-path-token fixture's 30000-segment token (agent-tasks c5810885) was measured under task 0089e6f5 and does not reach this: its full envelope is a few KB under the buffer on this repo's dev/CI platforms. Fixed by `writeJsonAndExit` (`src/cli.ts`), now used by both commands' `--json` path: the process exits only from the stdout write's own completion callback, so the whole payload is guaranteed to have reached the OS first, rather than exiting unconditionally right after scheduling the write. This also means a consumer piping either command's `--json` output must read stdout concurrently with the process running rather than waiting for it to exit first. A reader that closes the pipe early (`preflight run --json | head -c 100`) makes that write fail with EPIPE; direct measurement (Node 26, macOS, 25 runs across four reader shapes) found this always arrives at the write callback's own `err` argument, never as a `process.stdout` 'error' event, so the intended exit code (`0`/`1` from `result.ready`/`batchResult.notReady`) is kept from there. A `process.stdout` 'error' listener is still registered as a defensive fallback for a platform where the callback itself is never invoked, not because it is the path a real EPIPE takes here. Any other write failure (e.g. ENOSPC, EIO) writes a one-line diagnostic to stderr and exits non-zero instead of silently keeping the intended exit code or crashing with an uncaught exception. Both commands' non-JSON pretty-print paths still call `process.exit()` immediately after `console.log()` and are not touched by this fix.

- **A `npm-test` failure that only means "run the build first" is no longer a blocker** (agent-tasks c5810885). Reproduced against fixture repos in `tests/fixtures/`: a package whose test loads its own `dist/` output failed the default `npm-test` check with a bare `fail`/"npm test failed" in a fresh, unbuilt checkout, exactly the shape reported against a real repo whose CI always builds first. `runTestChecks` (`src/checks/test.ts`) now reports that as a named `skip` carrying the missing artifact and a remedy, and the decision is made by `evaluateBuildRequiredTestFailure` (`src/checks/shared.ts`) from three conditions that must ALL hold for EVERY package that failed. First, the filesystem precondition (`evaluateBuildPrecondition`): the package has a `build` script OF ITS OWN and at least one artifact it declares is missing on disk. A root fan-out (`npm run build --workspaces --if-present`) does not count for a workspace that has no build script: `--if-present` skips exactly such a workspace, and a fan-out without it would fail outright on one, so the workspace's own script is the only honest test. Declared artifacts are `main`, `module`, `types`/`typings`, `bin` (string or map values), the string targets of `exports` (subpath keys plus the `import`/`require`/`default`/`types` conditions, `*` patterns skipped) and the `outDir` of the package's own `tsconfig.json`, each resolved against the package directory, with Node's extension resolution applied to an extensionless entry point and a `dist/` fallback for a package that declares nothing. Second, corroboration (`findMissingArtifactEvidence`): that package's own output must blame the missing artifact, decided as a PATH rule rather than a text match. Every path-shaped token on a line (an absolute path, a `file://` URL, or a `./`/`../` specifier; a bare specifier such as `lodash` or `dist/index.js` is not one, since Node resolves those through `node_modules`) is resolved -- relative tokens against the failing package's own directory -- and accepted only when the resolved path IS the missing artifact, or is (or lies inside) the build-output directory that artifact identifies AND is itself not present on disk, is inside the repository, carries no `node_modules` segment, and belongs to the failing package rather than a neighbouring or nested one. Third, the package must not already be built (`evaluatePartialBuild`), which is a property of the PACKAGE and not of any one artifact: a package is PARTIALLY BUILT when any output directory it identifies holds an entry, or cannot be read at all. The directories it identifies are the directory of each declared artifact (`dist/index.js` identifies `dist`, a bare `dist` or a tsconfig `outDir` identifies itself, an artifact at the package root identifies none, since a whole package is not build output), plus `dist` when it identifies none of its own; a directory that canonicalizes outside the package is not read. Symlinks are resolved on both sides through the longest existing part of each path, so a package whose declared `dist` is a symlink to the directory its build really writes matches its own artifact; comparison is case-sensitive whatever the filesystem does; and a token past a hard bound (4096 characters or 256 path segments) is not resolved at all, since test output is untrusted input and every resolved path is walked segment by segment (an unbounded token used to abort the whole run with a `RangeError` and no JSON, which is now contained by an iterative walk, the bound, and a guard at the call sites that degrades an unanswerable corroboration to a blocking `fail` plus a `limitations` entry). That package-level condition is what keeps a PARTIALLY built package a blocker: a package can declare an artifact its build never emits (a `types` next to a JavaScript-only build, a dropped `exports` subpath, a `bin` that moved), so the precondition holds permanently while the real `dist/` is on disk and is what the tests load, and every failure of such a package -- a genuine runtime failure's own stack frame inside that built output, an `ENOENT` on an asset the build never copies, a `Cannot find module` for the never-emitted artifact itself -- used to corroborate as "not built yet" and stay `ready: true` even after a successful `npm run build`. Output already on disk means a build ran and did not produce the artifact, which no further build fixes, so such a package never downgrades. Reading the DIRECTORIES, and all of them, is what makes this a package property and covers the artifact reading as well (a declared artifact on disk necessarily makes its own directory non-empty): deciding it from the missing artifact's own directory read the wrong directory whenever a package declares output in more than one place, and both counter-shapes are ordinary -- a `main: dist/index.js` that is built beside a `types: dist/types/index.d.ts` that is never emitted (populated `dist/`, absent `dist/types/`), and `exports` naming `./dist/index.js` and `./lib/styles.css` where only `dist/` is written. Both are built, and reading one directory reported both as unbuilt and `ready: true`. The deliberate cost, unchanged in kind: any entry counts, so a STALE partial output directory (an older build missing a newly added entry), a checked-in file or `.keep`/`.gitkeep` placeholder, an OS or tool artefact such as a `.DS_Store`, and a file a test itself writes into the output directory during the run (the state is read at classification time, after the tests; nothing is snapshotted beforehand) all make the package read as partially built and block instead of skipping. So does a directory the package declares an artifact in that is not build output at all (a checked-in `bin/` launcher beside a `dist/` the build writes, an `exports` target inside a source directory), since the rule cannot tell source from output; there what fixes it is the declaration, not a build. A declared directory under `node_modules` is never read, the same rule the corroboration applies to paths: installed dependencies say nothing about whether the package was built. The remedy is otherwise the same build, and blocking is the safe direction for a verdict that opens push gates. The directory read goes through the filesystem, so on a case-insensitive filesystem a differently-cased declaration (`Dist/index.js`) reads the real `dist/`, which is the opposite of the case-sensitive PATH comparison in the corroboration and deliberately so: one asks the OS what is on disk, the other compares two strings. An output path that exists but cannot be read as a directory (a `dist` that is a FILE, a permission error) leaves the state unproven, which counts as built (the safe direction) and is reported as what it is (`could not be read (ENOTDIR)`) rather than as entries nobody counted, and that message claims nothing about what the output contains: it names the state, the artifact and the remedy (make the path a readable directory, then rebuild). The blocking message names the directory that decided it -- not necessarily the missing artifact's own -- the artifact, and the rebuild remedy, and it is used only when the failure actually names a path in that package's output; a partially built package whose failure names nothing of the kind keeps the plain "a declared build artifact (X) is missing, but the failure does not name it" sentence. That covers both real shapes, a package's own guard printing `<abs>/dist/index.js is missing` with no error prefix and Node's own `Cannot find module './dist/index.js'`/`ENOENT ... open '<path>'`, without either being decided by a substring: an earlier substring form accepted a stale relative require in an unbuilt package, a dependency missing under `node_modules/<lib>/dist/`, another workspace's artifact, and any runner stack frame through `node_modules/vitest/dist/`, each of them hiding a genuinely broken package behind `ready: true`. What stays indistinguishable by construction, and is documented rather than claimed away: a package with NO output on disk at all (every output directory it identifies absent or empty), failing on a path inside one of them a build would not create either (a stale reference to a renamed `dist/old.js`), which nothing on disk separates from "not built yet" until a build has run -- and the remedy the skip names resolves it either way, since after the build the output directory holds entries. No condition alone downgrades anything: without the precondition, one output line could hide a genuine failure (the previous, text-only classification did exactly that); without the corroboration, every failure in an unbuilt checkout would be downgraded, including in a package that compiles to `dist/` but runs its tests from source; and without the package-level condition, a package whose build never emits one of its declarations stays `ready: true` for every failure it ever has. An npm-workspaces fan-out is split per workspace (`splitWorkspaceSegments`, which now excludes the root package's own `> name@version script` preamble by identity and understands scoped names) and each failing workspace is resolved to its directory by package name, so one unbuilt workspace can never hide a different, genuinely broken one. Negative controls, each with a fixture: no build script anywhere; artifacts all present (including a module error for another file inside an already-built `dist/`); an unbuilt package whose failure does not name the artifact; a mixed monorepo in either workspace order; an unbuilt package failing on a stale relative require into its own source tree, alone and next to an unbuilt workspace; a missing dependency reported by a `node_modules` path whose tail equals the declared artifact; a package declaring no entry points whose assertion failure only carries a runner frame through `node_modules/<runner>/dist/`; a workspace corroborating on a neighbour's artifact; a workspace with no build script of its own under a root `--if-present` fan-out; an unparseable (JSONC) `tsconfig.json` whose failure names a directory other than the fallback artifact; seven partially built packages (a never-emitted `types`, a dropped `exports` subpath, a `bin` the build never emits while the test loads exactly it, a stale `types` next to a template the build never copies, that shape again as a workspace under a root `--if-present` fan-out, a `types` in a NESTED directory (`dist/types/`) beside a populated `dist/`, and an `exports` target in a SECOND output directory (`lib/`) beside a populated `dist/` -- the last two being exactly the shapes a per-artifact reading got wrong), each asserted in BOTH states: the named skip while nothing is built at all, a blocking `fail` after a successful build, and still that after a second build. Five of them name an absent path in their own output and are reported with the directory, the artifact and the remedy; the two whose failure is a genuine bug inside the live `dist/` (so the only path they name IS on disk) keep the plain "the failure does not name it" sentence, which is pinned in both directions; a package whose output directory holds a single placeholder file (blocking) next to the same fixture with an EMPTY output directory (the named skip); and a failing test printing a 30000-segment path token, asserted to leave the classification answering and the test failure the blocker (the CLI's own former 64 KB stdout truncation on such output was a separate, pre-existing limitation, fixed under task 0089e6f5). A package whose declared `dist` is a symlink to its real output directory has a fixture too, in both states, plus the third state where the `.keep` that lets git carry its empty output directory is left in place and the package therefore reads as partially built. The remedy names `npm run build` and `--setup` when the failing unit is the root package or the root build script fans out over the workspaces (the only cases where `--setup`'s root-level build reaches them), and a workspace-scoped `npm run build -w <name>` (or `--workspaces --if-present` for several) otherwise, so it never names a no-op. Scored exactly like any other `skip` (see `docs/confidence-scoring.md`): counted in the confidence denominator but not the numerator, plus the usual 0.03 `limitations` penalty; never scored as a `pass`. Scoped to the default auto-detected `npm run test` check; a `commands.test` override in `.preflight.json` is not classified.
